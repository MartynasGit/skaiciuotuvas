...
function App() {
  ...
  const ops = ['/', '*', '+', '-', '.']
  const buttonOps = ['/', '7', '8', '9', '*', '4', '5', '6', '+', '1', '2', '3', '-', '.', '0']
  ...
  return (
    <div className='calculator-grid'>
      ...
      <button className='span-two' onClick={clearCalc}>AC</button>
      <button onClick={deleteLast}>DEL</button>
      {buttonOps.map((keys) =>{
      return <button onClick={(e) => updateCalc(e.target.textContent)}>{keys}</button>}
      )}
